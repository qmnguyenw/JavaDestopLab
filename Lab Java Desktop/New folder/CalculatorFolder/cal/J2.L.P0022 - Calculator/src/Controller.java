...
public class Controller {
    ...
    void numberDotButtonPerformed(String numberDot) {
        isError=false;
        if(getValueScreen()==0) append = false;
    ...
    String formatNumber(double num) {
        ...
        if(numStr.length()>=16) {
            numStr = round16(numStr);
            System.out.println("hello2");
            numStr = roundToInt(numStr);
        }
        return numStr;
    }
    ...
    String roundToInt(String numberStr) {
        int dotIndex = numberStr.indexOf(".");
        if(numberStr.length() - dotIndex - 1 >= 5) {
            System.out.println("hello1");
            boolean allZero = true;
            boolean allNight = true;
            for (int i = dotIndex+1; i <= dotIndex+5; i++) {
                if(numberStr.charAt(i)!='0') {
                    allZero = false;
                }
                if(numberStr.charAt(i)!='9') {
                    allNight = false;
                }
            }
            if(allZero || allNight) {
                ...
                double number = Double.parseDouble(numberStr);
                long intNum = Math.round(number);
                numberStr = String.valueOf(intNum);
            }
        }
        return numberStr;
    }
