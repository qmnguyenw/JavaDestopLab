...
//                throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    ...
    public void itemNew() {
        ...
        if(!checkSaveContent.equals(textArea.getText())) doNewOperation = showAskSaveDialog();
    ...
    public void itemOpen() {
        boolean doOpen = true;
        if(!checkSaveContent.equals(textArea.getText())) doOpen = showAskSaveDialog();
        //if do open
        if(doOpen) {
            ...
            //if user cancel or close open dialog, not load file
            if (fileName==null) return;
            //current directory
            String dir = fileDialog.getDirectory();
            //set current file to the file user select in dialog
...
//                fin.close();
    ...
    public boolean itemSave() {
        ...
        if(currentFile==null) return itemSaveAsChooser();
        else {
            try {
    ...
    public boolean itemSaveAs() {
        ...
        String fileName = fileDialog.getFile();
        //if user cancel dialog then do not save
    ...
    public boolean itemSaveAsChooser() {
        int choice;
        File checkExistFile;
    ...
    public void itemExit() {
        boolean doExit = true;
        //if user has not save or have change content from last save, ask to save
        if(!checkSaveContent.equals(textArea.getText())) doExit = showAskSaveDialog();
